...
void binarySearch(FilesystemEntry* entries, uint32_t entriesLength, 
...
#if DEBUG_FS == 1
  clearscreen();
  dbgio_printf("*entries %p\n", entries);
  dbgio_printf("entriesLength %lu\n", entriesLength);
  dbgio_printf("searchElement %lu\n", searchElement.filenameHash);
  dbgio_printf("**foundEntry %p\n", *foundEntry);
#endif
...
#if DEBUG_FS == 1
  dbgio_printf("*pivot %p %lu\n", pivot, pivot->filenameHash);
  getinput();
#endif
  ...
  if (pivot->filenameHash == searchElement.filenameHash) {
    ...
    *foundEntry = pivot;
    ...
    if (pivot->filenameHash > searchElement.filenameHash) {
#if DEBUG_FS == 1
      dbgio_printf("*pivot > searchElement\n");      
#endif
...
#if DEBUG_FS == 1
  dbgio_printf("*record %p\n", record);
  dbgio_printf("parentHash %lu\n", parentHash); 
  dbgio_printf("parentPrime %lu\n", parentPrime);
  dbgio_printf("**entry %p\n", *entry); 
  dbgio_printf("*headerTable %p\n", headerTable);
  dbgio_printf("continueReading %d\n", continueReading);
  getinput();
#endif
...
#if DEBUG_FS == 1
    clearscreen();
    dbgio_printf("dir->identifierPtr() %.*s\n", (int)identifierSize, dir->identifierPtr());
    dbgio_printf("identifierSize %lu\n", identifierSize);
    dbgio_printf("parentHash %lu\n", parentHash);
    dbgio_printf("parentPrime %lu\n", parentPrime);
    dbgio_printf("&lastPrime %lu\n", lastPrime);
    dbgio_printf("hash %lu\n", hash);
    getinput();
#endif
...
//  assert(fsData != NULL);
//  navigateDirectory(fsData->root(), 0, printDirectoryRecord, NULL);
//}
...
#if DEBUG_READ_FILE_CURSOR == 1
    dbgio_printf("(If0) remainingBuffer %p %p %lu\n", fileCursor->cursor, fileCursor->sector.data, remainingBuffer);
    getinput();
#endif
